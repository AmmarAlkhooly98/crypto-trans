...
export const checkCmnd = async flags => {
	...
	if (flags?.date && !flags?.token && !flags?.date)
	...
	if (flags?.token && !flags?.date && !flags?.token)
	...
	if (flags?.token && flags?.date && (!flags?.token || !flags?.date))
		return console.log(
			info('INFO: Please provide a token symbol value and a date value')
		);
	else if (flags?.token && flags?.date) {
